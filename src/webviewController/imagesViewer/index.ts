        ...
        if (trimmed.length > 0) {
          try {
            ...
            if (fs.existsSync(resolved)) {
              ...
              // Scope to the hinted folder (or the hinted file's parent folder).
              // Allowed both inside the workspace (Explorer context actions) and
              // outside it (a standalone image opened via the custom editor).
              listScopeAbs = st.isDirectory() ? resolved : path.dirname(resolved)
            }
          } catch {
            //
          }
